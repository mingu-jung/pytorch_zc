...
/**
 * Note [CUDAZeroCopyAllocator design]
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  std::pair<void*, void*> allocate(size_t size) {
    ...
    if (primary_ctx_device_index.has_value()) {
      device_guard.reset_device(
          at::Device(at::DeviceType::ZC, *primary_ctx_device_index));
    }

    // Round up the allocation to the nearest power of two to improve reuse.
    void* ptr = nullptr;
    C10_CUDA_CHECK(cudaHostAlloc(
        &ptr, c10::llvm::PowerOf2Ceil(size), cudaHostAllocMapped));
...
static CUDAZeroCopyAllocator& getCUDAZeroCopyAllocator() {
  // leak and don't worry about shutdown
  static auto* r = new CUDAZeroCopyAllocator();
  return *r;
}

static void CUDAZeroCopyAllocatorDeleter(void* ctx) {
  getCUDAZeroCopyAllocator().free(ctx);
}

bool ZCHostAllocator_recordEvent(
    void* ptr,
    void* ctx,
    at::cuda::CUDAStream stream) {
  return getCUDAZeroCopyAllocator().record_event(ptr, ctx, stream);
}

// Releases cached pinned memory allocations via cudaHostFree
void ZCHostAllocator_emptyCache() {
  getCUDAZeroCopyAllocator().empty_cache();
}

struct CUDAZeroCopyAllocatorWrapper final : public at::Allocator {
  at::DataPtr allocate(size_t size) const override {
    auto ptr_and_ctx = getCUDAZeroCopyAllocator().allocate(size);
    return {
        ptr_and_ctx.first,
        ptr_and_ctx.second,
        &CUDAZeroCopyAllocatorDeleter,
        at::DeviceType::ZC};
  }
};

static CUDAZeroCopyAllocatorWrapper zc_host_allocator;

at::Allocator* getZCHostAllocator() {
  return &zc_host_allocator;
}
